...
typedef struct S_recipient { 
  int capacite;        
// Capacité totale du recipient 
  int volume;   
// Volume de ricard présent dans le récipient 
} T_recipient; 
...
int get_volume(T_recipient *recipient) // Retourne le volume de ricard présent dans le récipient 
{ 
    ...
    { 
      ...
      { 
        ...
        {                              // Et pour revenir une etape en arriere comme si il s'etait rien passé on restaure 
          tab_recipient[b].capacite = save_dst.capacite;      // la table des récipients a la valeur qu'elle avait avant l'appel a transvase() 
